...
function htmltodocx_html_allowed_children($tag = NULL) {
  ...
  $allowed_children = array(
    ...
    'br' => array(),
    'hr' => array(), // The tag name used for elements containing just text in SimpleHtmlDom.
...
function htmltodocx_insert_html_recursive(&$phpword_element, $html_dom_array, &$state = array()) {
  ...
  foreach ($html_dom_array as $element) {
    ...
    switch ($element->tag) {
      ...
      case 'h5':
      case 'h6':
		$s = trim(htmltodocx_clean_text($element->innertext));
		//if ($s === '') break;
        ...
        if ($state['structure_document'] && in_array($element->tag, array('h1', 'h2', 'h3', 'h4', 'h5', 'h6')) && is_object($state['phpword_object'])) {
          ...
          $phpword_element->addTitle($s, $state['structure_headings'][$element->tag]);
          break;
        }
        ...
        if (in_array($element->tag, $allowed_children)) {
          ...
          $state['textrun']->addText($s,  $state['current_style']);
        }
        unset($state['textrun']);
        if (!$space_after_set) {
      ...
      case 'a':
        ...
        if ($state['context'] == 'section') {
          ...
          // document:
          $state['textrun']->addLink(htmltodocx_url_encode_chars($href), $s, $state['current_style']);
        }
        else {
